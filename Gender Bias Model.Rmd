...
param_null <- list(cap_r = capital_r, low_r = lowercase_r)
...
head(null_data)
null_data %>% ggplot(aes(x=time, y=value, color=variable)) +
  geom_line() +
...
model_homo_free <- function (t, x, parms) {
  ...
  cap_r <- parms$cap_r
  low_r <- parms$low_r
  dx1dt <- (cap_r[1]*(1+low_r[1])*(b*x0)/((b*x0)+((1-b)*(1-x0)))) - (cap_r[1]*x1) - ((cap_r[1]*low_r[1]*(b*x1))/((b*x1)+((1-b)*(1-x1))))
  dx2dt <- (cap_r[2]*(1+low_r[2])*(b*x1)/((b*x1)+((1-b)*(1-x1)))) - (cap_r[2]*x2) - ((cap_r[2]*low_r[2]*(b*x2))/((b*x2)+((1-b)*(1-x2))))
  dx3dt <- (cap_r[3]*(1+low_r[3])*(b*x2)/((b*x2)+((1-b)*(1-x2)))) - (cap_r[3]*x3) - ((cap_r[3]*low_r[3]*(b*x3))/((b*x3)+((1-b)*(1-x3))))
  dx4dt <- (cap_r[4]*(1+low_r[4])*(b*x3)/((b*x3)+((1-b)*(1-x3)))) - (cap_r[4]*x4) - ((cap_r[4]*low_r[4]*(b*x4))/((b*x4)+((1-b)*(1-x4))))
  dx5dt <- (cap_r[5]*(1+low_r[5])*(b*x4)/((b*x4)+((1-b)*(1-x4)))) - (cap_r[5]*x5) - ((cap_r[5]*low_r[5]*(b*x5))/((b*x5)+((1-b)*(1-x5))))
  dx6dt <- (cap_r[6]*(1+low_r[6])*(b*x5)/((b*x5)+((1-b)*(1-x5)))) - (cap_r[6]*x6) - ((cap_r[6]*low_r[6]*(b*x6))/((b*x6)+((1-b)*(1-x6))))
  derivs <- c(dx1dt, dx2dt, dx3dt, dx4dt, dx5dt, dx6dt)
  list(derivs)
}
...
param_homo <- list(cap_r = capital_r, low_r = lowercase_r, bias=b)
...
#only weird thing is when b=0.35, theres a weird dip for the lower levels in the early years
...
for(i in seq_along(biases_to_test)){
  ...
  parameter <- list(cap_r = capital_r, low_r = lowercase_r, bias=bias_i)
...
param_bias <- list(cap_r = capital_r, low_r = lowercase_r, lambda=lambda)
param_bias
...
for(i in seq_len(trials)){
  b <- bias_test[i]
  l <- homophily_test[i]
  params <- list(cap_r = capital_r, low_r = lowercase_r, lambda = l, bias = b)
